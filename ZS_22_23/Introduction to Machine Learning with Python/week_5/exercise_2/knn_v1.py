...
def main(args: argparse.Namespace):
    ...
    for i in range(test_data.shape[0]):
        nearest_dist = np.zeros(args.k)
        distances = np.linalg.norm(test_data[i] - train_data, args.p, axis=1)
        nearest = np.argpartition(distances, args.k)
        for j in range(len(nearest_dist)):
            nearest_dist[j] = distances[int(nearest[j])]
        votes = np.zeros(10)
        for j in range(args.k):
            ...
            if args.weights == "uniform":
                ...
                if nearest_dist[j] == 0:
                    ...
                    weight = 1 / nearest_dist[j]
            elif args.weights == "softmax":
                weight = (scipy.special.softmax(- nearest_dist))[j]
            votes[int(train_target[int(nearest[j])])] += weight
