...
# fasttext embedding:
#ft = fasttext.load_model('cc.cs.64.bin')
#fasttext.util.reduce_model(ft, 64)
#ft.save_model('cc.cs.64.bin')
#print(ft.get_nearest_neighbors('pes'))
#print(ft.get_word_vector('pes'))
...
class Model(tf.keras.Model):
    ...
    def __init__(self, args: argparse.Namespace, train: MorphoDataset.Dataset) -> None:
        ...
        #words = tf.keras.layers.Input(shape=[None], dtype=tf.float32, ragged=True)
        ...
        #emb_words = ft.get_word_vector(hidden)
        #emb_words = hidden.map(ft.get_word_vector)
        #emb_words = hidden
...
def main(args: argparse.Namespace) -> None:
    ...
    def create_dataset(name):
        ...
        #dataset = dataset.map(ft.get_word_vector)
