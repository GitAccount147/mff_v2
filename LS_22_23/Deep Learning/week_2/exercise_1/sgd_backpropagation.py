...
class Model(tf.Module):
    ...
    def predict(self, inputs: tf.Tensor) -> tf.Tensor:
        ...
        ins = ins @ self._W1 + self._b1
        ins = tf.nn.tanh(ins)
        ins = ins @ self._W2 + self._b2
        return tf.nn.softmax(ins)
    ...
    def train_epoch(self, dataset: MNIST.Dataset) -> None:
        for batch in dataset.batches(self._args.batch_size):
            ...
            with tf.GradientTape() as tape:
                ...
                #one_hot = tf.one_hot(batch["labels"], 10)
                #sums = one_hot @ tf.transpose(probabilities)
                #loss = tf.math.reduce_mean(-tf.math.log(tf.linalg.diag_part(sums)))


                labels = tf.convert_to_tensor(batch["labels"], dtype=tf.int32)
                p_data = tf.gather(probabilities, labels, batch_dims=1)
                loss = tf.math.reduce_mean(-tf.math.log(p_data))
