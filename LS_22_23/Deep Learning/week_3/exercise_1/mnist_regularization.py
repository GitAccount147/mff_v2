...
# Zly Pepa (mozna obcas i vtipny? prej ne): 3d76595a-e687-11e9-9ce9-00505601122b
# "nooooooooooooooo", "ja se s tebou nebudu bavit", "bez do haje": 594215cf-e687-11e9-9ce9-00505601122b
...
def main(args: argparse.Namespace) -> Dict[str, float]:
    ...
    mnist.train.data["labels"] = tf.keras.utils.to_categorical(
        mnist.train.data["labels"], num_classes=10, dtype='float32')
    mnist.dev.data["labels"] = tf.keras.utils.to_categorical(
        mnist.dev.data["labels"], num_classes=10, dtype='float32')
    mnist.test.data["labels"] = tf.keras.utils.to_categorical(
        mnist.test.data["labels"], num_classes=10, dtype='float32')

    mnist.train.data["labels"] = mnist.train.data["labels"] * (1 - args.label_smoothing) + (args.label_smoothing / 10)
    mnist.dev.data["labels"] = mnist.dev.data["labels"] * (1 - args.label_smoothing) + (args.label_smoothing / 10)
    mnist.test.data["labels"] = mnist.test.data["labels"] * (1 - args.label_smoothing) + (args.label_smoothing / 10)
