...
parser.add_argument("--cnn", default="5-3-1,10-3-1", type=str, help="CNN architecture.")  # "5-3-2,10-3-2"
...
class Convolution:
    ...
    def backward(
        ...
        mid_layer = 0 #derivace

        #print("outputs:", outputs.shape)
        #print("outputs grad:", outputs_gradient.shape)
        B, o1, o2, o3 = outputs_gradient.shape  # ~G
        bias_gradient = tf.zeros(shape=[B, o3])
        for i in range(o1):
            for j in range(o2):
                out_grad_prime = outputs_gradient[:,i,j,:]
                out_prime = outputs[:,i,j,:]
                out_norm = tf.math.divide_no_nan(out_prime, tf.math.abs(out_prime))
                thru_relu = tf.math.maximum(tf.zeros(out_prime.shape), out_norm)
                bias_gradient += tf.math.multiply(out_grad_prime, thru_relu)

        bias_gradient = tf.reduce_sum(bias_gradient, axis=0)
        #bias_gradient = tf.math.maximum(tf.zeros(bias_gradient.shape), bias_gradient)

        k = self._kernel_size
        B, H, W, C = inputs.shape
        O = self._filters
        #kernel_gradient = tf.zeros(shape=[B, k, k, C, O])
        kernel_gradient = tf.Variable(tf.initializers.Zeros()([B, k, k, C, O]))  # ~K
        s = self._stride
            ...
            for n in range(k):
                for i in range(o1):  # o1
                    for j in range(o2):  # o2
                        ...
                        G = outputs_gradient[:,i,j,:]

                        out_prime = outputs[:, i, j, :]
                        out_norm = tf.math.divide_no_nan(out_prime, tf.math.abs(out_prime))
                        thru_relu = tf.math.maximum(tf.zeros(out_prime.shape), out_norm)
                        G_prime = tf.math.multiply(G, thru_relu)

                        #ein = tf.einsum("ab,ac->abc", I, G)
                        ein = tf.einsum("ab,ac->abc", I, G_prime)
                        e1, e2, e3 = ein.shape
                        #print("ein shape:", ein.shape)
                        #kernel_gradient[:,m,n,:,:] += ein
                        ind = tf.constant([[a, m, n] for a in range(B)])
                        kernel_gradient = tf.tensor_scatter_nd_add(kernel_gradient, indices=ind, updates=ein)
                        #kernel_gradient += tf.reshape(ein, [B, e1, e2, e3, O])

        kernel_gradient_old = kernel_gradient
        ...
        G_prime = tf.math.multiply(outputs_gradient, thru_relu)
        G_pad = tf.pad(G_prime, [[0, 0], [k-1, k-1], [k-1, k-1], [0, 0]])
