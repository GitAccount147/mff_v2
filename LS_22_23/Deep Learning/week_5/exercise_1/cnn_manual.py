...
class Convolution:
    ...
    def backward(
        ...
        if s == 1:
            ...
            G_new = tf.Variable(tf.initializers.Zeros()([B, H, W, O]))
            for i in range(o1):
                for j in range(o2):
                    ind4 = tf.constant([[a, i*s, j*s] for a in range(B)])
                    G_new = tf.tensor_scatter_nd_add(G_new, indices=ind4, updates=G[:,i,j,:])
