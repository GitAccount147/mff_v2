...
def main(args: argparse.Namespace) -> None:
    ...
    def mask_to_float(image: tf.Tensor, mask_1: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:
        return image, tf.cast(mask_1, tf.float32) * (1/256)
    ...
    train_dataset = train_dataset.map(mask_to_float)
    ...
    dev_dataset = dev_dataset.map(mask_to_float)
    ...
    test_dataset = test_dataset.map(mask_to_float)
