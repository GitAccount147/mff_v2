...
def main(args: argparse.Namespace) -> None:
    ...
    tconv = tf.keras.layers.Conv2DTranspose(kernel_size=2, strides=2, filters=3, padding='same')(relu2)
    batch_norm3 = tf.keras.layers.BatchNormalization()(tconv)
    relu3 = tf.keras.activations.relu(batch_norm3)
    ...
    conv3 = tf.keras.layers.Conv2D(kernel_size=3, filters=3, padding='same')(relu3)
    batch_norm4 = tf.keras.layers.BatchNormalization()(conv3)
    relu4 = tf.keras.activations.relu(batch_norm4)
    ...
    model.compile(
        optimizer=optimizer,
        loss=tf.losses.BinaryFocalCrossentropy(gamma=args.gamma),
