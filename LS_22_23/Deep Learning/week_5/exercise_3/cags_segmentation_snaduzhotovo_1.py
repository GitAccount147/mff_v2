...
# new:
parser.add_argument("--weight_decay", default=0.004, help="value of weight_decay")
parser.add_argument("--learning_rate", default=0.001, help="value of learning_rate")
parser.add_argument("--weight_decay2", default=0.004, help="value of weight_decay2")
parser.add_argument("--learning_rate2", default=0.001, help="value of learning_rate2")
parser.add_argument("--use_bias", default=True, help="use_bias in CONV")
...
def main(args: argparse.Namespace) -> None:
    ...
    conv1 = tf.keras.layers.Conv2D(kernel_size=3, filters=backbone.outputs[0].shape[3], padding='same',
                                   input_shape=(7,7,1280), use_bias=args.use_bias)(backbone.outputs[0])
    batch_norm1 = tf.keras.layers.BatchNormalization()(conv1)
    relu1 = tf.keras.activations.relu(batch_norm1)
    conv2 = tf.keras.layers.Conv2D(kernel_size=3, filters=backbone.outputs[0].shape[3], padding='same',
                                   input_shape=(7,7,1280), use_bias=args.use_bias)(relu1)
    ...
    for i in range(5):
        # up
        tconv = tf.keras.layers.Conv2DTranspose(kernel_size=2, strides=2, filters=layers[i].shape[3], padding='same',
                                                use_bias=args.use_bias)(relu2)
        ...
        #add = tf.keras.layers.Add()([relu3, layers[i + 1]])

        # right
        conv3 = tf.keras.layers.Conv2D(kernel_size=3, filters=layers[i+1].shape[3], padding='same', use_bias=args.use_bias)(concat)
        batch_norm4 = tf.keras.layers.BatchNormalization()(conv3)
        relu4 = tf.keras.activations.relu(batch_norm4)
        conv4 = tf.keras.layers.Conv2D(kernel_size=3, filters=layers[i+1].shape[3], padding='same', use_bias=args.use_bias)(relu4)
    ...
    #print(flat_conv)
    flat_conv = tf.keras.layers.Conv2D(kernel_size=1, filters=1, input_shape=(224, 224, 3), activation=tf.nn.sigmoid, padding='same')(relu2)
    ...
    optimizer2 = tf.optimizers.experimental.AdamW(learning_rate=0.0001)
