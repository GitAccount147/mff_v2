...
parser.add_argument("--gamma", default=2.0, type=float, help="Gamma for Loss")
parser.add_argument("--bin", default=False, help="Binary Loss")
...
def main(args: argparse.Namespace) -> None:
    ...
    backbone = tf.keras.applications.EfficientNetV2B0(include_top=False, input_tensor=tf.keras.Input([CAGS.H, CAGS.W,CAGS.C]))
    ...
    conv1 = tf.keras.layers.Conv2D(kernel_size=3, filters=backbone.outputs[1].shape[3], padding='same', input_shape=(7,7,1280))(backbone.outputs[0])
    batch_norm1 = tf.keras.layers.BatchNormalization()(conv1)
    relu1 = tf.keras.activations.relu(batch_norm1)
    '''conv2 = tf.keras.layers.Conv2D(kernel_size=3, filters=backbone.outputs[1].shape[3], padding='same', input_shape=(7,7,112))(relu1)
    batch_norm2 = tf.keras.layers.BatchNormalization()(conv2)
    relu2 = tf.keras.activations.relu(batch_norm2)'''


    for i in range(1,5):
        # up (1280, 112, 48, 32, 16, 3)
        tconv = tf.keras.layers.Conv2DTranspose(kernel_size=2, strides=2, filters=layers[i].shape[3], padding='same')(relu1)
        batch_norm3 = tf.keras.layers.BatchNormalization()(tconv)
        relu3 = tf.keras.activations.relu(batch_norm3)

        concat = tf.keras.layers.Add()([relu3, layers[i]])
        #concat = tf.keras.layers.Concatenate()([relu3, layers[i]])
        #print("relu3:", relu3.shape)
        #print("layers[i]:", layers[i].shape)
        #print("concat:",concat.shape)

        # right
        conv3 = tf.keras.layers.Conv2D(kernel_size=3, filters=layers[i+1].shape[3], padding='same')(concat)
        batch_norm4 = tf.keras.layers.BatchNormalization()(conv3)
        relu1 = tf.keras.activations.relu(batch_norm4)
    ...
    #print(tconv.shape)
    tconv = tf.keras.layers.Conv2DTranspose(kernel_size=2, strides=2, filters=16, padding='same')(relu1)
    batch_norm3 = tf.keras.layers.BatchNormalization()(tconv)
    relu3 = tf.keras.activations.relu(batch_norm3)

    #concat = tf.keras.layers.Concatenate()([relu3, layers[5]])
    #print("relu3:", relu3.shape)
    #print("layers[i]:", layers[5].shape)
    #print("concat:", concat.shape)

    conv3 = tf.keras.layers.Conv2D(kernel_size=3, filters=8, padding='same')(relu3)
    batch_norm4 = tf.keras.layers.BatchNormalization()(conv3)
    relu3 = tf.keras.activations.relu(batch_norm4)
    '''conv4 = tf.keras.layers.Conv2D(kernel_size=3, filters=3, padding='same')(relu4)
    batch_norm5 = tf.keras.layers.BatchNormalization()(conv4)
    relu2 = tf.keras.activations.relu(batch_norm5)'''

    #flat_conv = tf.keras.layers.Conv2D(kernel_size=1, filters=1)(relu2)
    #print(flat_conv)
    flat_conv = tf.keras.layers.Conv2D(kernel_size=1, filters=1, input_shape=(224, 224, 8),
                                       activation=tf.nn.sigmoid, padding='same')(relu3)
    #flatten = tf.keras.layers.Flatten(input_shape=(224, 224, 1))(flat_conv)
    #print(flat_conv.shape)
    #out = tf.keras.layers.Dense(224*224, activation=tf.nn.sigmoid, input_shape=(224*224,1))(flatten)
    #out = tf.keras.layers.Dense(256, activation=tf.nn.sigmoid, input_shape=(224*224,1))(flat_conv)

    #out = tf.math.sigmoid(tf.reshape(flat_conv, [-1,224,224]))

    #print(flat_conv.shape)

    model = tf.keras.Model(inputs=backbone.inputs, outputs=flat_conv)

    optimizer = tf.optimizers.experimental.AdamW(learning_rate=args.learning_rate, weight_decay=args.weight_decay)
