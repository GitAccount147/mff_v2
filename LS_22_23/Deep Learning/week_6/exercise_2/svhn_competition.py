...
# new:
parser.add_argument("--supr_thr", default=0.5, type=float, help="No max supression threshold.")
parser.add_argument("--back_thr", default=0.25, type=float, help="Background threshold.")
parser.add_argument("--supr_cnt", default=1, type=int, help="Supression max boxes.")
...
def main(args: argparse.Namespace) -> None:
    ...
    #dev_eval = svhn.dev.map(lambda x: (x["image"], x["classes"], x["bboxes"]))
    dev_eval = svhn.dev.map(lambda x: (x["image"]))
    ...
    def resize_dev(img): # cls, box
        scale = 224 / tf.shape(img)[0]
        dev_scale_vector.append(scale)
        return tf.image.resize(img, [224, 224])#, (cls, box)

    train = train.map(resize_all)
    dev = dev.map(resize_all)
    dev_eval = dev_eval.map(resize_dev)
    ...
    for i in range(new_size):
        for j in range(new_size):
            anchors.append(tf.constant([max(anch_size * i - anch_size // 2, 0), anch_size * j,
                                        min(anch_size * (i + 1) + anch_size // 2, 223), anch_size * (j + 1)]))
    ...
    if take:
        ...
        #dev_eval = dev_eval.take(take_count)
        test = test.take(take_count)

    train = train.batch(args.batch_size)
    dev = dev.batch(args.batch_size)
    dev_eval = dev_eval.batch(args.batch_size)
    ...
    class_head = tf.keras.layers.Conv2D(kernel_size=3, strides=1, padding="same", filters=filt)(backbone.outputs[layer])
    class_head = tf.keras.layers.BatchNormalization()(class_head)
    class_head = tf.keras.activations.relu(class_head)
    class_head = tf.keras.layers.Conv2D(kernel_size=3, strides=1, padding="same", filters=10 * A,
                                        activation=tf.math.sigmoid)(class_head)
    class_head = tf.keras.layers.Reshape((new_size * new_size, 10 * A), input_shape=(new_size, new_size, 10 * A),
                                         name="class_head")(class_head)
    # class_head = tf.keras.layers.Reshape((49*10 * A,1), input_shape=(7, 7, 10 * A), name="class_head")(class_head)

    box_head = tf.keras.layers.Conv2D(kernel_size=3, strides=1, padding="same", filters=filt)(backbone.outputs[layer])
    ...
    box_head = tf.keras.layers.Reshape((new_size * new_size, 4 * A), input_shape=(new_size, new_size, 4 * A),
                                       name="box_head")(box_head)
    ...
    def postprocess(test_pred, scale_vector):
        ...
        keep = np.where(max_matrix >= args.back_thr)
        ...
        scales = tf.stack(scale_vector[1:])
        scales = scales.numpy()
        ...
        for i in range(scales.shape[0]):
            ...
            for num in np.unique(class_for_img):
                ...
                selected_indices = tf.image.non_max_suppression(
                    bx, sc, max_output_size=tf.constant(args.supr_cnt), iou_threshold=args.supr_thr)
    ...
    dev_pred = model.predict(dev_eval)
    test_pred = model.predict(test)

    dev_predicted = postprocess(dev_pred, dev_scale_vector)
    dev_acc = svhn.evaluate(svhn.dev, dev_predicted)
    print(dev_acc)
    #print("dev_predicted:", dev_predicted[:10])

    out = postprocess(test_pred, test_scale_vector)
