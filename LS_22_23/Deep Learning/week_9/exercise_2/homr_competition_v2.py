...
parser.add_argument("--cnn_arch",
                    default="CB-8-3-1-same,CB-8-3-1-same,M-3-2,CB-16-3-1-same,CB-16-3-1-same,M-3-2,F,H-100",
...
# "CB-8-3-1-same,CB-8-3-1-same,M-3-2,CB-16-3-1-same,CB-16-3-1-same,M-3-2,M-3-2,M-3-2,M-3-2,M-3-2,M-3-2"
# "CB-8-3-3-1-1-same,CB-8-3-3-1-1-same,M-5-5-3-3,CB-16-3-3-1-1-same,CB-16-3-3-1-1-same,M-5-3-3-2,M-5-3-3-2,M-3-3-2-2,M-3-1-2-1"
...
class MargeSimpson(tf.keras.Model):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        layers = args.cnn_arch.split(',')
        strides = []
        for i in range(len(layers)):
            params = layers[i].split("-")
            layer_type = params[0]
            layer = ...
            if layer_type == "CB":  # 2D convolution with batch normalization
                # e.g.: CB-32-3-3-2-2-same
                conv = tf.keras.layers.Conv2D(filters=(int(params[1]), int(params[2])),
                                              kernel_size=(int(params[3]), int(params[4])),
                                              strides=(int(params[5]), int(params[6])),
                                              padding=params[7], use_bias=False)
                strides.append(int(params[6]))
                bn = tf.keras.layers.BatchNormalization()
                activation = tf.nn.relu
                hidden = activation(bn(conv(hidden)))
            elif layer_type == "C":  # 2D convolution
                layer = tf.keras.layers.Conv2D(filters=(int(params[1]), int(params[2])),
                                               kernel_size=(int(params[3]), int(params[4])),
                                               strides=(int(params[5]), int(params[6])),
                                               padding=params[7], use_bias=False,
                                               activation=tf.nn.relu)
                ...
                layer = tf.keras.layers.MaxPool2D(pool_size=(int(params[1]), int(params[2])),
                                                  strides=(int(params[3]), int(params[4])))
                strides.append(int(params[4]))
