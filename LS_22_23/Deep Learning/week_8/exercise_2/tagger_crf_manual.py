...
parser.add_argument("--rnn", default="LSTM", choices=["LSTM", "GRU"], help="RNN layer type.")  # "LSTM"
...
class Model(tf.keras.Model):
    ...
    def compute_third_loss(self, logits, seq_len):
        ...
        for i in range(batch_size):
            ...
            for j in range(1, cur_seq_len):
                ...
                inp = tf.transpose(self._crf_weights) + logits_init  # need to broadcast
                lse = tf.math.reduce_logsumexp(input_tensor=inp, axis=1)
                logits_init = lse + new_vec

            add_val = tf.math.reduce_logsumexp(input_tensor=logits_init, axis=0)
            add_vec = tf.one_hot(indices=i, depth=batch_size)*add_val
            out += add_vec
    ...
    def crf_log_likelihood(self, logits, gold, sequence_lengths):
        ...
        for i in range(batch_size):
            ...
            l1_vec += tf.one_hot(indices=i, depth=tf.cast(batch_size, dtype=tf.int32))*loss_to_add
            ...
            loss_to_add = tf.reduce_sum(gathered)
            l2_vec += tf.one_hot(indices=i, depth=tf.cast(batch_size, dtype=tf.int32))*loss_to_add

        # THIRD LOSS:
        third_loss = self.compute_third_loss(logits=logits, seq_len=sequence_lengths)

        return l1_vec + l2_vec - third_loss
