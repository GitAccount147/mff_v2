...
class Agent:
    ...
    def train(self, states: np.ndarray, actions: np.ndarray, returns: np.ndarray) -> None:
        ...
        with tf.GradientTape() as tape1:
            pred_baseline = self._baseline(states, training=True)  # training=True
            ...
            #loss_baseline = self._baseline.loss(returns, pred_baseline)


        with tf.GradientTape() as tape2:
            prob = self._model(states, training=True)  # training=True
            loss = self._model.compute_loss(states, tf.one_hot(actions, 2), prob, sample_weight=delta)
            #loss = self._model.loss(tf.one_hot(actions, 2), prob, sample_weight=delta)
        ...
        #self._model.optimizer.minimize(loss, self._model.variables, tape=tape2)
        #self._baseline.optimizer.minimize(loss_baseline, self._baseline.variables, tape=tape1)
